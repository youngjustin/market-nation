...
public class RandomNumberJobTest {
    ...
    private final static long RANDOM_NUMBER = 3;

    @Mock
    private RandomNumberSupplier randomNumberSupplier;

    @Mock
    private InputNumberRepository inputNumberRepository;

    @Mock
    private RabbitMQSender rabbitMQSender;

    @InjectMocks
    private RandomNumberJob randomNumberJob;
    ...
    void run() {
        ...
        Mockito.verify(this.inputNumberRepository, times(1)).save(argThat(x -> x.getNumber() == RANDOM_NUMBER));
        Mockito.verify(this.rabbitMQSender, times(1)).send(argThat(x -> x instanceof InputNumberEntity && ((InputNumberEntity)x).getNumber() == RANDOM_NUMBER));
